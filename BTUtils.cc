...
//		pieceFreqVector.push_back(PieceFreqEntry::PieceFreqEntry(i,0));
	    PieceFreqEntry pfeObject(i,0);
	    pieceFreqVector.push_back(pfeObject);
	}
...
void PieceFreqState::printPieceFrequencies()
{
	...
	{
		ev<<"\t\tIndex = "<<(*it).getIndex()<<" Frequency = "<<(*it).frequency()<<endl;
	}
}
...
//PeerEntry::PeerEntry(PEER, TCPServerThreadBase*,  simtime_t    connTime)
//{
//    this->peerInfo = peer;
//    this->thread = thread;
//    this->connTime_var = SIMTIME_DBL(connTime);
//}
...
//method Added By Manoj
void PeerEntry::setConnTime(  SimTime   connTime)
{
    connTime_var = SIMTIME_DBL(connTime);
}
...
//method Added By Manoj
void PeerState::addPeer(PEER peer, TCPServerThreadBase* thread,const SimTime & connTime)
{
    addPeer(peer, thread, SIMTIME_DBL(connTime));
}
...
void PeerState::removePeer(string id)
{
	...
	if ( index>-1 )
	{
		...
		opp_error("BTUtils: Peer state management problem. Peer %s not found!", id.c_str());
}
...
void PeerState::print()
{
	for (unsigned int i=0; i<peerVector.size(); i++)
	{
		...
		cerr<<"\tPeer entry #"<<(i+1)<<" "<<peerVector[i].getPeerID()<<", state =  "<<threadBT->getState()<<", Choked: " <<threadBT->peerChoking()<<endl;
...
void RequestEntry::setBitLength(int l)
{
	length_var = l;
}

simtime_t RequestEntry::getTimestamp()
{
...
{
	addRequest(re.getIndex(),re.begin(),re.length(),re.getTimestamp(),re.peerID());
}
...
{
	return findRequest(entry.getIndex(),entry.begin());
}

int RequestState::findRequest(int index, int begin)
{
	for (unsigned int i=0; i<requestVector.size(); i++)
			if ((requestVector[i].getIndex() == index) &&  (requestVector[i].begin() == begin))
				return i;
...
{
	removeRequest(entry.getIndex(),entry.begin(), silently);
}
...
void RequestState::print()
{
	for (unsigned int i=0; i<requestVector.size(); i++)
	{
		cerr<<"\t\tRequest entry #"<<(i+1)<<" ("<<requestVector[i].getIndex()<<","<<requestVector[i].begin()<<", "<<requestVector[i].getTimestamp()<<")"<<endl;
		ev<<"\t\tRequest entry #"<<(i+1)<<" ("<<requestVector[i].getIndex()<<", "<<requestVector[i].begin()<<", "<<requestVector[i].getTimestamp()<<")"<<endl;
	}
}
...
int RequestState::maxBlockRequested(int pieceIndex)
{
	...
	for (unsigned int i=0; i<requestVector.size(); i++)
	{
		if ((requestVector[i].getIndex() == pieceIndex) && (requestVector[i].begin()>maxBlockRequested))
...
{
//	int minBlockRequested = std::numeric_limits<int>::getMax();
    int minBlockRequested = std::numeric_limits<int>::max();
	for (unsigned int i=0; i<requestVector.size(); i++)
	{
		if ((requestVector[i].getIndex() == pieceIndex) && (requestVector[i].begin()<minBlockRequested))
			minBlockRequested = requestVector[i].begin();
	}

//	if (minBlockRequested < std::numeric_limits<int>::getMax())
