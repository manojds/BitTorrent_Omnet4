...
{
	numRemainingPieces_var--;
	setNumRemainingBlocks(std::max(numRemainingBlocks()-numBlocks(),0));
...
bool BitField::update(int bitfieldIndex, int blockfieldIndex,bool expected)
{
	...
	currentBlock_var[bitfieldIndex] = std::max(blockfieldIndex,currentBlock_var[bitfieldIndex]);
	...
	setNumRemainingBlocks(std::max(numRemainingBlocks()-1,0));
	setNumRequestedBlocks(std::max(numRequestedBlocks()-1,0));
	...
	{
		bitfield[bitfieldIndex] = true;
		setNumRemainingPieces(std::max(numRemainingPieces()-1,0));
		return true;
	}
	else
		return false;
...
int BitField::setBlockRequested(int pieceIndex,int blockIndex, bool value)
{
	...
	if (value)
	{
		...
		setNumNonRequestedNonAvailableBlocks(std::max(numNonRequestedNonAvailableBlocks() - 1,0));
...
{
	--(currentlyRequestedPieceBlocks_var[bitfieldIndex]);
	currentlyRequestedPieceBlocks_var[bitfieldIndex] = std::max(currentlyRequestedPieceBlocks_var[bitfieldIndex],0);
}
