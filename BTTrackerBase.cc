...
{
    BT_LOG_ESSEN(btLogSinker, "BTTrackerB::writeStats", "******** Tracker Stats ******** - Time ["<<simTime() <<"] Started Count ["<<started_count_var<<
            "], Current peer count ["<<peersNum()<<"] seeder count ["<<seeds()<<"] completed count ["<<completed_count_var<<"]");
}
